...
function* SagaGetMoreActiveList() {
  while (WORKING) {
    ...
    try {
      const result = yield call([API, API.get], '/venalink/participate', payload);

      yield put({ type: SUCCESS_GET_MORE_ACTIVE_VENALINK_LIST, payload: result });
    }
...
function* SagaGetMoreShareList() {
  while (WORKING) {
    ...
    try {
      ...
      yield put({ type: SUCCESS_GET_MORE_SHARE_VENALINK_LIST, payload: result });
    }
...
export default function* venalink() {
  yield [
