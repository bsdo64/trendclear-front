...
function* SagaSubmitSubComment() {
  while (WORKING) {
    ...
    try {
      const result = yield call([Api, API.post], '/community/subComment',
        payload);
      const normalized = normalize(result, subComment);
      yield put({
        type: SUCCESS_SUBMIT_SUB_COMMENT,
        normalized,
...
function* SagaUpdateSubComment() {
  while (WORKING) {
    ...
    try {
      const result = yield call([Api, API.put], '/community/subComment',
        payload);
      const normalized = normalize(result, subComment);
