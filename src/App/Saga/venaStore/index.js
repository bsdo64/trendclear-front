...
function* SagaParticipateVenalink() {
  while (WORKING) {
    ...
    try {
      const result = yield call([API, API.post], '/venalink/participate',
        payload);
      result.inventories = normalize(result.inventories, inventory);

      yield put(
        { type: SUCCESS_PARTICIPATE_VENALINK, result, postId: payload.postId });
    }
...
function* SagaPurchaseItem() {
  while (WORKING) {
    ...
    try {
      const result = yield call([API, API.post], '/venastore/purchase/item',
        payload);
      result.inventories = normalize(result.inventories, inventory);
